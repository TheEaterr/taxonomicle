	...
	<article class="prose m-5 inline-block max-w-[120ch] bg-base-100">
		...
		<div class="text-justify">
			...
			<p>
				...
				website. The tree of life in itself was extracted from Wikidata, using information from the <a
				...
				>. In short, this project gives a centralised and computer friendly way to store taxonomic
				data and use it accross Wikimedia projects. For example, it is what powers the taxonomic
				data block found in every taxon Wikipedia page. This dataset also gives access to images for
				the different taxons, through Wikimedia Commons. Finally, the description for each taxon was
			...
			<span role="alert" class="alert max-w-screen-lg border-base-300 bg-base-300">
				...
				<span>
					...
					<span class="inline-block text-justify"
						...
						possible - an image that define them. Wikipedia being an encyplopedia is the only source
			...
			<p>
				...
				than coherent. For example, many taxa have several possible parents and possible ranks. The
				first step in building the dataset is to make it into a tree. For this, we choose for each
				taxon the parent that gives it the longest path back to animalia (the node at the top of the
				tree). This was found to be best as it gave the most exhaustive results. All taxa that have
				no ranks were also removed. This allows preventing the game from being too hard and tedious,
			...
			<span role="alert" class="alert max-w-screen-lg border-base-300 bg-base-300">
				...
				<span>
					...
					<span class="inline-block text-justify"
						...
						presence in the data were kept. The ranks in Taxonomicle are: kingdom, subkingdom,
			...
			<p>
				This sadly doesn't give a coherent tree and more than a hundred manual data fixes (either
				...
				inherently partial and Taxonomicle doesn't try to reach any scientific accuracy.
				Nevertheless, if a taxon is parent of another in Taxonomicle, it can be assumed this
				hierarchy is reasonable (though maybe debated or outdated). The main problem that wasn't
			...
			<p>
				...
				selecting a random species only mostly gives insects and gastropods and doesn't offer much
